# %%
"""Recovery metrics: MCC, Dead Latents, Uniqueness vs L0 — 4 SAE architectures × 8 benchmarks.
...
# Rows 1–3: MCC, Dead Latents, Uniqueness for distributions 1–4
# Row 4: spacer
# Rows 5–7: MCC, Dead Latents, Uniqueness for distributions 5–8
N_M = len(METRICS)
N_COLS = 4
N_ROWS = 2 * N_M + 1  # 7
...
for _, row_off in _GROUPS:
    for mi, (yrange, ydtick, yminor) in enumerate(zip(_Y_RANGES, _Y_DTICKS, _Y_MINOR)):
        for ci in range(1, N_COLS + 1):
            fig.update_yaxes(
                range=yrange,
                dtick=ydtick,
                minor=dict(
                    dtick=yminor, showgrid=True, gridcolor="#F0F0F0", gridwidth=1
                ),
                row=row_off + mi + 1,
                col=ci,
            )
...
# --- Save ---
_fd = fig_dir()
fig.write_image(f"{_fd}/recovery_metrics.pdf")
fig.write_image(f"{_fd}/recovery_metrics.svg")
print(f"Saved to {_fd}/recovery_metrics.{{pdf,svg}}")
