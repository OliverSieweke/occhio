...
# Dead Latents row: auto-range
for bi in range(N_REAL_COLS):
    fig.update_yaxes(
        range=None, dtick=None, minor=dict(showgrid=False), row=2, col=_data_col(bi)
    )
...
# --- Save ---
_fd = fig_dir()
fig.write_image(f"{_fd}/recovery_metrics.pdf", engine="kaleido")
fig.write_image(f"{_fd}/recovery_metrics.svg", engine="kaleido")
